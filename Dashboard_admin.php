...
// PERBAIKAN: Ambil jumlah rumah terjual dari tb_unit
// Asumsi: Kolom status di tb_unit menyimpan 'Terjual' untuk unit yang sudah lunas
$sql_terjual = "SELECT COUNT(*) AS terjual FROM tb_unit WHERE status = 'Terjual'";
$result_terjual = $conn->query($sql_terjual);
$row_terjual = $result_terjual->fetch_assoc();
$rumah_terjual = $row_terjual['terjual'];

// PERBAIKAN: Ambil jumlah rumah terbooking dari tb_unit
// Asumsi: Kolom status di tb_unit menyimpan 'Terbooking' untuk unit yang sedang dipesan
$sql_terbooking = "SELECT COUNT(*) AS terbooking FROM tb_unit WHERE status = 'Terbooking'";
...
// Bagian ini tetap mengambil dari tb_pembelian karena menampilkan riwayat transaksi,
// yang mungkin memiliki detail seperti nama pembeli, tanggal pembelian, dll.,
// yang tidak ada di tb_unit.
        ...
        <main class="main-content">
            ...
            <div class="content-area">
                <div class="dashboard-overview">
                    ...
                    <i class="fas fa-home"></i>
                        <h3>Total Unit Rumah</h3>
                        <span class="value"><?php echo $total_rumah_unit; ?></span>
                    </div>
                    <div class="overview-card">
                        <i class="fas fa-check-circle"></i>
                        <h3>Unit Terjual</h3> <span class="value"><?php echo $rumah_terjual; ?></span>
                    </div>
                    <div class="overview-card">
                        <i class="fas fa-calendar-check"></i>
                        <h3>Unit Terbooking</h3> <span class="value"><?php echo $rumah_terbooking; ?></span>
                    </div>
                </div>
    ...
    <ul class="activity-list">
        ...
        if ($result_status_properti->num_rows > 0) {
            while ($row = $result_status_properti->fetch_assoc()) {
                ...
                $status_color = '';
                // Pastikan status di tb_pembelian sesuai dengan yang Anda gunakan
                if ($status == 'terjual') { // Sesuaikan dengan status Lunas di tb_pembelian
                    $status_color = 'red';
                } elseif ($status == 'terbooking') { // Sesuaikan dengan status Terbooking di tb_pembelian
                    $status_color = 'green';
                } else {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (notificationData && notificationData.type === 'success') {
            if (notificationMessage && notificationContainer) {
